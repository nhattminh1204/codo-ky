...
| **Auth & User Profile** | 🟢 Hoàn thiện một phần (Auth SDK ready) | `auth_provider.dart` gọi `FirebaseAuth` & `FirebaseFirestore`. Có error mapping Firebase khá đầy đủ. `test/unit/auth_provider_test.dart` pass. Tuy nhiên `.env.dev` sử dụng Firebase config mẫu (`FIREBASE_API_KEY`). `profile_home_screen.dart:20-34` hardcode thông tin fallback user ("Nguyễn Văn Minh Nhật", "nhattminh1204@gmail.com") khi user null/loading. | Trung bình | - Đảm bảo key Firebase production/dev hợp lệ<br>- Loại bỏ các fallback mock hardcoded trong Profile screen khi chưa login |
