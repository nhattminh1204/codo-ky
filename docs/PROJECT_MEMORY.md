...
- **02/08/2026**: Hoàn thành tối ưu hóa toàn bộ hệ thống giao diện MapBottomSheet và TravelModePicker: (1) Phân tách chuẩn 2 trạng thái chọn phương tiện: Giữ nguyên 100% giao diện 3 nấc nằm ngang khi thu gọn/vuốt xuống và kích hoạt 3D Infinite Wheel Dial / `VehicleWheelPicker` nằm bên trái nút Chỉ Đường khi mở rộng/vuốt lên; (2) Tối ưu hóa layout `VehicleWheelPicker` căn chỉnh tỉ lệ `viewportFraction: 0.333` trên khung chứa `145px` loại bỏ hoàn toàn lỗi trào viền icon; (3) Tích hợp phản hồi rung nấc khấc `HapticFeedback.selectionClick()` kết hợp `lightImpact()` mô phỏng xoay bánh xe đặt báo thức iPhone; (4) Kích hoạt `PointerDeviceKind.mouse` và `AppScrollBehavior` trên Windows Desktop cho phép kéo lướt chuột trái mượt mà; (5) Sửa triệt để lỗi `LateInitializationError` bằng getter lười null-safe `_controller`. Bộ kiểm thử `flutter test` pass 90/90 tests (100%). Các file cập nhật: `app.dart`, `travel_mode_picker.dart`, `vehicle_wheel_picker.dart`, `map_bottom_sheet.dart`, `map_bottom_sheet_widget_test.dart`, `DECISIONS_LOG.md`, `PROJECT_MEMORY.md`.
